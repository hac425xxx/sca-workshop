...
int ntohl_wrapper(int n)
{
    int x = ntohl(n);
    return x;
}

int return_const(int n)
{
    int x = ntohl(n);
    return 42;
}

void bad_memcpy(int a, char *buf)
{
    int n = ntohl_wrapper(a);
    char *s = malloc(100);
    memcpy(s, buf, n);
}

void good_memcpy(int a, char *buf)
{
    int n = return_const(a);
    char *s = malloc(100);
    memcpy(s, buf, n);
}
